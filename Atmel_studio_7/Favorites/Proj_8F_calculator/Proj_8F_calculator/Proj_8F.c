...
/*Proj_8F_calculator	Explores the Atmel Studio maths and IO functions
**********************************************************************/
...
while(1){
	...
	*/
	...
	digit_entry = 1;
	for(int m = 0; m<=7; m++){disp_bkp[m]=digits[m]; digits[m]=0;}
	...
	while(switch_1_down)
		{scroll_display_zero();Timer_T0_10mS_delay_x_m(20);}
	...
	switch(digits[0]){
		case '-':	break;
		case '.':	scroll_control &= (~(0x3)); break;				//exponent & d.p. disabled
