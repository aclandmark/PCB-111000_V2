...
int uart_getfloat_from_csv(FILE *mystr_input);

FILE  uart_output = FDEV_SETUP_STREAM (uart_putchar, NULL, _FDEV_SETUP_WRITE);
FILE uart_input_any_char = FDEV_SETUP_STREAM(NULL, uart_getchar, _FDEV_SETUP_READ);
FILE uart_input_float_from_csv = FDEV_SETUP_STREAM(NULL, uart_getfloat_from_csv, _FDEV_SETUP_READ);
...
{float Floating_point_num, accumulator_A, accumulator_G;
	...
	if ((MCUSR & (1 << PORF)) || MCUSR & (1 << EXTRF)){MCUSR = 0;
		...
		setup_PC_comms_Basic(0,103);getchar();
		printf("Send data files\rArithmetic mean\tGeometric mean\t data items");
	SW_reset;}
	...
	num_counter = 0;
	stdout = &uart_output;
	stdin  = &uart_input_float_from_csv;
	accumulator_G = 0;
	accumulator_A = 0;
	while(1){scanf("%f", &Floating_point_num);
...
/***********************************************************************************************************/	
	int uart_getchar_Local(FILE *mystr_input)									//Special version
	{int n = 0, m = 5;
		while (!(UCSR0A & (1 << RXC0)))
