...
int main (void) {
  ...
  if (Arduino_non_WDTout) {                           //EEPROM 0x3EE holds anything except zero
    ...
    if ((switch_2_down) || (MCUSR & (1 << EXTRF)))
...
Press sw3 and enter time or sw2 to reset it to zero, then press sw1 to start the clock.\r\n");
    ...
    while (1) {
      if (switch_2_down)
      {
        clock_mode = 2;  //Select timer or clock
        break;
      }
      if (switch_3_down) {
        clock_mode = 1;
  ...
  if (Arduino_WDTout) {                                     //Arduino_WDTout
    ...
    if ((switch_3_down) && (switch_1_down)) {                 //User induced watch dog timeout:
      while ((switch_3_down) || (switch_1_down)) {              //Use sw_1 and sw_3 to adjust time
        if ((switch_3_up) && (switch_1_down)) {
          sec_counter += 1;
        }
        if ((switch_1_up) && (switch_3_down)) {
          sec_counter -= 1;
        }
    ...
    }                            //Displays time as adjusted by user
    ...
    I2C_initiate_10mS_ref(); sei();
  }                       //Reinitialise 10mS tick
    ...
    switch (clock_mode) {
      ...
      case 'E':                                               //Clock ready to start: Awaiting sw3 press
        if (switch_1_up); else {
          ...
          sei();
          clock_mode = 'B';
          setup_64ms_WDT_with_interrupt;
        }                       //User input finished: Start WDT
        break;

      case 'B': if (!(TWI_flag));
        else {
          TWI_flag = 0; Update_sec_counter;                  //Clock running mode
          old_clock_mode = 'B';
      ...
      case 'F': if (!(TWI_flag));
          ...
          TWI_flag = 0; Update_sec_counter;                  //Stop watch mode
          old_clock_mode = 'F';
          if (((sec_counter) > 1) && ((switch_1_down)))
          {
            clock_resume_time = restore_Clock();
            clock_mode = 'B';
          }
          else Timer();
        }
      ...
      case 'H': if (!(TWI_flag));
          ...
          TWI_flag = 0;   Update_sec_counter;               //Stop watch restored mode
          old_clock_mode = 'H';
...
void Timer(void) {                                     //Calls "update_7_seg_display", to close one I2C transaction and sets "payload" to initiate the next one
  ...
  if (!(ms_counter % 100)) {                            //When 1 second tick occurs
    ...
    update_7_seg_display();
  }                              //Update display and complete the current I2C transaction
  if (ms_counter == 99)payload = 8; else payload = 2;
}       //Set payload for next TWI_interrupt 8 for 1sec tick, 2 for 10mS tick
...
ISR(TWI_vect) {                                         //Detects 10mS interrupt. Initiates I2C_transaction and responds by sending payload size
  ...
  if (!(ms_counter)) {
    sec_counter_V++;                     //Volatile version of "sec_counter" increments every second
    if (sec_counter_V == 86400)sec_counter_V = 0;           //Roll-over for 24 hour clock
    sec_counter_save = 1;
  }                                     //Enables "sec_counter" to be updated from "sec_counter_V"
...
} //No payload? Prepare for next 10ms interrupt
...
void update_7_seg_display(void) {
  ...
  {
    digits[0] = digits[1];  //Optional: masks 10mS digit
    digits[1] = 0;
  }
  ...
  for (int m = 0;  m < payload; m++) {
    ...
    if (m == (payload - 1))TWCR = (1 << TWINT) | (1 << TWEN); //last byte being transmitted now expect NACK in response
      ...
      TWCR = (1 << TWINT) | (1 << TWEA) | (1 << TWEN); //ACK pulse normally required
    }
    while (!(TWCR & (1 << TWINT)));
  }                          //Clear interrupt
  TWCR =
...
}                                          //Corrupt clock_mode variable: set to default value and restart
...
long restore_Clock(void) {                                 //Restores time by adding "clock_time" to the current SW time
  ...
  Serial.write("\r\n"); return clock_restore_time;
}          //Return "clock_restore_time" to "main loop"
...
long restore_stop_watch() {                               //Restores SW time by subtracting the current time from the value stored in "clock_time"
  ...
  return SW_restore_time;
}                                  //Return "SW_restore_time" to "main loop"
...
}                                     //Resets global variable "display_clear"
